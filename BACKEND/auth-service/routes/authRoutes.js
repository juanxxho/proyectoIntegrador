...
router.get(
  ...
  (req, res) => {
    res
      .status(200)
      .json({ message: "Acceso a la vista de administrador", user: req.user });
  }
);
...
router.get(
  "/jefe-dashboard",
  verifyToken,
  verifyRole(["jefe de brigada"]),
  (req, res) => {
    res
      .status(200)
      .json({
        message: "Acceso a la vista de jefe de brigada",
        user: req.user,
      });
  }
);
...
router.get(
  ...
  (req, res) => {
    res
      .status(200)
      .json({ message: "Acceso a la vista de botánico", user: req.user });
  }
);
...
router.get(
  ...
  (req, res) => {
    res
      .status(200)
      .json({ message: "Acceso a la vista de auxiliar", user: req.user });
  }
);

// Ruta protegida solo para coinvestigadores
router.get(
  "/coinvestigador-dashboard",
  verifyToken,
  verifyRole(["coinvestigador"]),
  (req, res) => {
    res
      .status(200)
      .json({ message: "Acceso a la vista de coinvestigador", user: req.user });
  }
);
