...
const asignarBrigada = async (req, res) => {
  try {
    ...
    if (result[0] > 0) {
      res
        .status(200)
        .json({ message: "Brigada asignada a investigación", result });
    } else {
      res.status(404).json({ message: "Investigación no encontrada" });
    }
  } catch (error) {
...
const obtenerPorBrigada = async (req, res) => {
  const { id_brigada } = req.params;

  try {
    const investigacion = await Investigacion.findOne({
      where: { brigada_id: id_brigada },
    });

    if (!investigacion) {
      return res
        .status(404)
        .json({ mensaje: "No se encontró investigación para esta brigada" });
    }

    res.json(investigacion);
  } catch (error) {
    console.error("Error al buscar investigación por brigada:", error);
